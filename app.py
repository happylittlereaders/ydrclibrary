...
app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "default-fallback-secret-key")
...
def get_db_client():
    try:
        ...
        if firestore_keys:
            ...
            # Local fallback or direct file fallback if environment variable is missing
            return None
...
def load_data():
    ...
    try:
        ...
        # Create Unified Text Field for Fuzzy Search
        ...
        # Fetch Covers dynamically or lazy load
        cover_urls = []
...
def index():
    df, c = load_data()
    if df.empty:
        return "Failed to load library database.", 500
    ...
    # Lightweight CPU-friendly RapidFuzz Match
    if f_fuzzy:
    ...
    # Extract distinct Interest Level choices
...
def book_detail(book_idx):
    ...
    # Fetch Cloud Firestore Comments
    comments = []
    if db:
        try:
            ...
            comments = sorted(comments, key=lambda x: x.get('time', ''), reverse=True)
        except Exception as e:
            print(f"Error fetching comments: {e}")

    return render_template("detail.html", row=row, idx=c, book_idx=book_idx, comments=comments, user=session.get("user"))
...
def register():
    ...
    if not validate_email(email) or len(password) < 6:
        flash("Invalid email or password under 6 characters.", "danger")
        return redirect(url_for("index"))
